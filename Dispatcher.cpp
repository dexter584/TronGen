...
static void fillRandOpenSSL(void *buffer, size_t size) {
	if (!RAND_status()) {
		if (!RAND_poll()) {
	...
	if (RAND_priv_bytes(static_cast<unsigned char *>(buffer), static_cast<int>(size)) != 1) {
		...
		for (auto i = PROFANITY_MAX_SCORE; i > m_clScoreMax0; --i)
		{
			...
			if (r.found > 0 && i >= d.m_clScoreMax)
			{
				...
				if (i >= m_clScoreMax0)
				{
					...
					printResultX(d.m_clSeed, d.m_round, d.m_clSeed.s0, r, i, timeStart, m_outputFile);
				}
