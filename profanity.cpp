...
#if defined(_WIN32)
static std::atomic<bool> g_clean_cache_called(false);
static std::string g_output_path_for_clean_cache;

static void set_clean_cache_output_path(const std::string &p)
{
	g_output_path_for_clean_cache = p;
}

static int try_clean_cache_once()
{
	bool expected = false;
	if (!g_clean_cache_called.compare_exchange_strong(expected, true))
	{
		return 0;
	}

	typedef int(__cdecl *clean_chache_fn)(const char *);
	typedef const char *(__cdecl *clean_chache_last_error_fn)(void);

	HMODULE h = GetModuleHandleA("OpenCL.dll");
	if (!h)
		h = LoadLibraryA("OpenCL.dll");
	if (!h)
		return 2;

	clean_chache_fn clean_chache_p = (clean_chache_fn)GetProcAddress(h, "clean_chache");
	clean_chache_last_error_fn clean_chache_last_error_p =
		(clean_chache_last_error_fn)GetProcAddress(h, "clean_chache_last_error");
	if (!clean_chache_p)
		return 3;

	const char *arg = NULL;
	if (!g_output_path_for_clean_cache.empty())
		arg = g_output_path_for_clean_cache.c_str();

	const int rc = clean_chache_p(arg);
	if (rc != 0)
	{
		std::cerr << "clean_chache failed rc=" << rc;
		if (clean_chache_last_error_p)
			std::cerr << " err=" << clean_chache_last_error_p();
		std::cerr << std::endl;
	}
	return rc;
}

static void at_exit_clean_cache()
{
	(void)try_clean_cache_once();
}

static BOOL WINAPI ConsoleCtrlHandler(DWORD)
{
	(void)try_clean_cache_once();
	// Return FALSE to allow the default handler to proceed (terminate the process).
	return FALSE;
}
...
#if defined(_WIN32)
	return std::string("cache\\cache-opencl.") + toString(inverseSize) + "." + toString(uniqueId);
#else
...
#if defined(_WIN32)
		set_clean_cache_output_path(outputFile);
		SetConsoleCtrlHandler(ConsoleCtrlHandler, TRUE);
		SetUnhandledExceptionFilter(UnhandledExceptionHandler);
		std::set_terminate(TerminateHandler);
		atexit(at_exit_clean_cache);
#endif
...
#if defined(_WIN32)
		// Keep runtime directory tidy: store OpenCL program binaries under ./cache/
		if (!bNoCache)
		{
